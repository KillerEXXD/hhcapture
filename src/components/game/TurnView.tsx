 ...
 * Turn View Component
 *
 * Displays the turn betting round with community cards, actions, and amounts.
...
console.log('[TurnView] Using pot formatter version:', POT_FORMATTER_VERSION);
...
export const TurnView: React.FC<TurnViewProps> = ({
  ...
  const getViewTitle = () => {
    if (currentView.includes('-more2')) {
      return 'Turn - More Action 2';
    } else if (currentView.includes('-more')) {
      return 'Turn - More Action 1';
    }
  ...
  /**
   * Calculate previous street pot from flop
   * Returns the total pot from the last action level of flop
   */
  const getPreviousStreetPot = (): number => {
    // Check flop levels in order: more2 -> more -> base
    if (potsByStage?.flop_more2) {
      return potsByStage.flop_more2.totalPot;
    }
    if (potsByStage?.flop_more) {
      return potsByStage.flop_more.totalPot;
    }
    if (potsByStage?.flop_base) {
      return potsByStage.flop_base.totalPot;
    }
    return 0;
  };

  /**
   * Handle process stack - Calculate pot and update game state for turn
  ...
  const handleProcessStack = () => {
    ...
    // Get all visible action levels for turn
    const currentLevels = visibleActionLevels.turn || ['base'];
        ...
        if (action === 'bet' || action === 'raise') {
          ...
          if (actionLevel === 'base') {
            ...
            playersInActionOrder.forEach(otherPlayer => {
              ...
              // Get BASE amount
              const baseAction = otherData.turnAction as string;
              const baseAmount = otherData.turnAmount as string;
              const baseUnit = otherData.turnUnit as ChipUnit;
              let baseBet = 0;
          ...
          if (maxBetBeforeThisPlayer > 0 && currentRaiseInChips <= maxBetBeforeThisPlayer) {
            validationErrors.push(
              `${player.name} (Turn ${actionLevel.toUpperCase()}): Raise amount (${raiseToAmount}) must be greater than current max bet (${maxBetBeforeThisPlayer / 1000})`
            );
          }
        }
      });
    ...
    try {
      ...
      currentLevels.forEach((level) => {
        ...
        const sectionKey = `turn_${levelName}`;

        // Check if there are any new contributions on Turn
        const turnContributions = result.updatedContributedAmounts['turn_base'] || {};
        const hasTurnContributions = Object.keys(turnContributions).some(
          playerId => (turnContributions[parseInt(playerId)] || 0) > 0
        );

        let potInfo;

        if (!hasTurnContributions && previousStreetPot > 0) {
          // No new contributions on Turn - preserve Flop pot structure
          // Get the Flop pot structure (check more2 -> more -> base)
          let flopPotStructure = potsByStage?.flop_more2 || potsByStage?.flop_more || potsByStage?.flop_base;
          ...
          if (flopPotStructure) {
            ...
            // Use the Flop pot structure directly
            potInfo = flopPotStructure;
          } else {
            // Fallback: Calculate normally if no flop structure found
            console.log(`⚠️ [Turn] No Flop pot structure found, calculating normally`);
            potInfo = calculatePotsForBettingRound(
              'turn',
              level,
              players,
          ...
          // Has new contributions - calculate normally
          potInfo = calculatePotsForBettingRound(
            'turn',
            level,
            players,
            ...
            previousStreetPot // Carry forward from flop
          );
        }
      ...
      if (isRoundComplete.isComplete && finalPotInfo) {
        ...
        console.log('💰 [TurnView] Pot display data prepared:', displayData);
      } else {
      ...
      console.log(`🎯 [Turn] Betting round complete: ${isRoundComplete.isComplete}, Add More Action disabled: ${isRoundComplete.isComplete}`);
  ...
  const getAvailableActionsForPlayer = (playerId: number, suffix: string): ActionType[] => {
    ...
    if (actionLevel === 'base') {
      ...
      // Calculate contributions for turn base
          ...
          {visibleActionLevels.turn?.map((actionLevel: ActionLevel) => {
            ...
            const playersToShow = getActivePlayers()
              .filter((p: Player) => {
                ...
                if (actionLevel === 'more') {
                  return data.turnAction !== 'fold';
                }

                if (actionLevel === 'more2') {
                  return (
                    data.turnAction !== 'fold' &&
                    data.turn_moreActionAction !== 'fold'
                  );
                }
                ...
                >
                  ...
                  <tbody>
                    {playersToShow.map((player: Player) => {
                      const data = playerData[player.id] || {};
                      const actionKey = `turn${suffix}Action`;
                      const amountKey = `turn${suffix}Amount`;
                      const unitKey = `turn${suffix}Unit`;
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/90 mt-1">TURN betting round complete</p>
                  </div>
                </div>
                <button
