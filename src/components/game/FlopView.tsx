        ...
        if (action === 'bet' || action === 'raise') {
          ...
          if (actionLevel === 'base') {
            ...
            playersInActionOrder.forEach(otherPlayer => {
              ...
              // Get BASE amount
              const baseAction = otherData.flopAction as string;
              const baseAmount = otherData.flopAmount as string;
              const baseUnit = otherData.flopUnit as ChipUnit;
              let baseBet = 0;
          ...
          if (maxBetBeforeThisPlayer > 0 && currentRaiseInChips <= maxBetBeforeThisPlayer) {
            validationErrors.push(
              `${player.name} (Flop ${actionLevel.toUpperCase()}): Raise amount (${raiseToAmount}) must be greater than current max bet (${maxBetBeforeThisPlayer / 1000})`
            );
          }
        }
      });
