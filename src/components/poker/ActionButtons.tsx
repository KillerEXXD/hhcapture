 ...
 * - Auto-focus amount input when bet or raise is clicked
...
export const ActionButtons: React.FC<ActionButtonsProps> = ({
   ...
   * Focuses amount input for bet/raise actions, or triggers onActionComplete for others
   */
  const handleActionClick = (action: ActionType) => {
    ...
    if (action === 'bet' || action === 'raise') {
      ...
      // For all other actions, notify parent to handle navigation
      setTimeout(() => {
    ...
    return (
      ...
      >
        <div className="flex flex-col items-center justify-center leading-tight">
          ...
          <div className="text-[10px] opacity-70 mt-0.5">{action.shortcut}</div>
        </div>
      </button>
    );
  };
