...
print('=' * 80)
print('VALIDATION REPORT - 40 Test Cases')
print('=' * 80)
print()

for tc_num, tc_content in tc_matches[:40]:
    errors = []
    warnings = []
    ...
    if results_match:
        ...
        # FIXED: Split table into individual rows to avoid multi-row matching bug
        table_rows = re.findall(r'<tr>.*?</tr>', results_table, re.DOTALL)

        for player in players:
            # Find the row for this specific player
            player_row = None
            for row in table_rows:
                if f'{player["name"]} (' in row:
                    player_row = row
                    break
